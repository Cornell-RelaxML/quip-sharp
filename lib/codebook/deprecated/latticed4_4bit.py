...
def get_grid():
    ...
    gd4 = torch.cartesian_prod(*[hintr] * _D44B_CODESZ)
    gd4m2 = (gd4.sum(dim=-1) % 2 == 0)
    gd4n = gd4.norm(dim=-1)**2 <= 161
    d4 = gd4[torch.where(gd4m2 * gd4n)[0]]
    return d4


_D44B_CACHED = get_grid()
_D44B_NORM_CACHED = torch.diag(_D44B_CACHED @ _D44B_CACHED.T)


class D44B_codebook(nn.Module):

    def __init__(self, build_truncated=True):
        super(D44B_codebook, self).__init__()
        self.opt_scale = 3.77
        self.codesz = _D44B_CODESZ
        self.idx_dtype = torch.int16
        self.idx_offset = -2**15

        grid = _D44B_CACHED
        self.register_buffer('grid', grid)
        self.register_buffer('grid_norm', _D44B_NORM_CACHED)
        ...
        if build_truncated:
            ...
            self.register_buffer('int_map', 2**torch.arange(_D44B_CODESZ))
...
class QuantizedD44BLinear(nn.Module):

    def __init__(self, device):
        super().__init__()
        self.codebook = D44B_codebook(build_truncated=False).to(device)
    ...
    def forward(self,
        ...
        x = input.view(-1, n * _D44B_CODESZ).to(torch.float32)
        if rescale_WH:
            x /= scaleWH
        x = x * SU
        ...
        W_decompressed = self.codebook.by_idxs(Qidxs).reshape(-1, n * _D44B_CODESZ)

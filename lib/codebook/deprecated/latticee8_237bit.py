...
def get_grid():
    intr = torch.arange(-5, 5)
    hintr = intr + 1 / 2

    gintr = torch.cartesian_prod(*[intr] * _E8237B_CODESZ)
    ghintr = torch.cartesian_prod(*[hintr] * _E8237B_CODESZ)

    ge8 = torch.concat([gintr, ghintr], dim=0)
    ge8m2 = (ge8.sum(dim=-1) % 2 == 0)
    ge8n = ge8.norm(dim=-1)**2 <= 18

    e8 = ge8[torch.where(ge8m2 * ge8n)[0]]
    return e8

_E8237B_CACHED = get_grid()
_E8237B_NORM_CACHED = _E8237B_CACHED.norm(dim=-1)**2

class E8237B_codebook(nn.Module):

    def __init__(self, build_truncated=True):
        super(E8237B_codebook, self).__init__()
        self.opt_scale = 1.26
        self.codesz = _E8237B_CODESZ
        self.idx_dtype = torch.int32

        grid = _E8237B_CACHED
        self.register_buffer('grid', grid)
        self.register_buffer('grid_norm', _E8237B_NORM_CACHED)
        ...
        if build_truncated:
            ...
            self.register_buffer('int_map', 2**torch.arange(_E8237B_CODESZ))
        ...
        '''        
        self.cuda()
        samples = torch.distributions.multivariate_normal.MultivariateNormal(torch.zeros(8), torch.eye(8)).rsample([200000]).cuda()
        print(samples.shape)
        def fn_s(s):
    ...
    def iterate_mask(self, device=0):
        ...
        for i in range(len(allcombo)):
            allcombo_idx[i] = (
                self.round(allcombo[i], self.grid.to(device), self.grid_norm.to(device))[1]).to(self.idx_dtype)
...
class QuantizedE8237BLinear(nn.Module):

    def __init__(self, device):
        super().__init__()
        self.codebook = E8237B_codebook(build_truncated=False).to(device)
    ...
    def forward(self,
        ...
        x = input.view(-1, n * _E8237B_CODESZ).to(torch.float32)
        if rescale_WH:
            x /= scaleWH
        x = x * SU
        ...
        W_decompressed = self.codebook.by_idxs(Qidxs).reshape(-1, n * _E8237B_CODESZ)

...
def get_grid():
    ...
    gd4 = torch.cartesian_prod(*[hintr] * _D4234B_CODESZ)
    gd4m2 = (gd4.sum(dim=-1) % 2 == 0)
    gd4n = gd4.norm(dim=-1)**2 <= 15.1 # should be 15
    d4 = gd4[torch.where(gd4m2 * gd4n)[0]]
    return d4


_D4234B_CACHED = get_grid()
_D4234B_NORM_CACHED = torch.diag(_D4234B_CACHED @ _D4234B_CACHED.T)


class D4234B_codebook(nn.Module):

    def __init__(self, build_truncated=True):
        super(D4234B_codebook, self).__init__()
        self.opt_scale = 1.46
        self.codesz = _D4234B_CODESZ
        self.idx_dtype = torch.int16
        self.idx_offset = -2**15

        grid = _D4234B_CACHED
        self.register_buffer('grid', grid)
        self.register_buffer('grid_norm', _D4234B_NORM_CACHED)
        ...
        if build_truncated:
            ...
            self.register_buffer('int_map', 2**torch.arange(_D4234B_CODESZ))
        ...
        '''
        import numpy as np
        assert np.abs(np.log2(self.grid.shape[0])/4 - 2.34) < 1e-2
        self.cuda()
...
class QuantizedD4234BLinear(nn.Module):

    def __init__(self, device):
        super().__init__()
        self.codebook = D4234B_codebook(build_truncated=False).to(device)
    ...
    def forward(self,
        ...
        x = input.view(-1, n * _D4234B_CODESZ).to(torch.float32)
        if rescale_WH:
            x /= scaleWH
        x = x * SU
        ...
        W_decompressed = self.codebook.by_idxs(Qidxs).reshape(-1, n * _D4234B_CODESZ)
